...
class FederatedModelEvaluator:
    ...
    def evaluate(
        ...
        max_store = None if full_metrics else int(max(64, max_samples_for_periodic_auc))
        ...
        if not full_metrics and total_samples > 0:
            ...
            cls_metrics.f1_weighted = float(((f1_0 * (tn + fp)) + (f1_1 * (tp + fn))) / float(total_samples + 1e-12))
