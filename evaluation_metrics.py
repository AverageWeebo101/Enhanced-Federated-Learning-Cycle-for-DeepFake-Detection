...
def measure_inference_latency(
    ...
    batched = test_data.batch(batch_size).prefetch(1)
...
class FederatedModelEvaluator:
    ...
    def evaluate(
        ...
        max_samples_for_periodic_auc: int = 2048,
    ) -> EvaluationReport:
        """
        ...
        # Stream metrics to avoid materialising the entire test set in RAM.

        y_true_list: List[np.ndarray] = []

        y_prob_list: List[np.ndarray] = []

        max_store = None if full_metrics else int(max(32, max_samples_for_periodic_auc))
        ...
        for batch in batched:
            ...
            y_np = np.asarray(y.numpy()).ravel()

            p_np = np.asarray(preds.numpy()).ravel()

            y_bin = (y_np >= 0.5).astype(np.int32)

            p_bin = (p_np >= threshold).astype(np.int32)

            tp += int(np.sum((y_bin == 1) & (p_bin == 1)))

            tn += int(np.sum((y_bin == 0) & (p_bin == 0)))

            fp += int(np.sum((y_bin == 0) & (p_bin == 1)))

            fn += int(np.sum((y_bin == 1) & (p_bin == 0)))

            total_samples += int(y_bin.size)
            ...
            elif stored < max_store:

                keep = min(max_store - stored, int(y_np.size))

                y_true_list.append(y_np[:keep])

                y_prob_list.append(p_np[:keep])
        ...
        # Override with exact streaming totals in lightweight periodic mode.

        if not full_metrics and total_samples > 0:

            acc = (tp + tn) / float(total_samples)

            prec1 = tp / float(tp + fp + 1e-12)

            rec1 = tp / float(tp + fn + 1e-12)

            f1_1 = 2.0 * prec1 * rec1 / float(prec1 + rec1 + 1e-12)

            prec0 = tn / float(tn + fn + 1e-12)

            rec0 = tn / float(tn + fp + 1e-12)

            f1_0 = 2.0 * prec0 * rec0 / float(prec0 + rec0 + 1e-12)

            cls_metrics.accuracy = float(acc)

            cls_metrics.precision_macro = float((prec0 + prec1) / 2.0)

            cls_metrics.recall_macro = float((rec0 + rec1) / 2.0)

            cls_metrics.f1_macro = float((f1_0 + f1_1) / 2.0)

            cls_metrics.f1_weighted = float(
                ((f1_0 * (tn + fp)) + (f1_1 * (tp + fn))) / float(total_samples + 1e-12)
            )

            cls_metrics.num_samples = int(total_samples)

            cls_metrics.confusion_matrix = [[int(tn), int(fp)], [int(fn), int(tp)]]

            if len(self.class_names) >= 2:

                cls_metrics.f1_per_class = {
                    self.class_names[0]: float(f1_0),
                    self.class_names[1]: float(f1_1),
                }

            cls_metrics.roc_auc = float(cls_metrics.roc_auc) if y_true.size > 0 else 0.0
        ...
        if run_latency:
            ...
            lat_metrics = LatencyMetrics(batch_size=batch_size)
        ...
        logger.info(
            ...
            f"{size_metrics.total_params:,}",
            size_metrics.file_size_mb,
        )
    ...
    def save_report(
        ...
        logger.info("Reports saved to %s and %s", json_path.name, txt_path.name)
    ...
    def save_comparison_report(
        ...
        logger.info(
            "Comparison report saved to %s and %s", json_path.name, txt_path.name
        )
