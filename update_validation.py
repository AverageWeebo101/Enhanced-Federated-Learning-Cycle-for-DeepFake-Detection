...
class ClippingConfig:
    ...
    clip_value: Optional[float] = (
        None  # if set, clip *to* this norm instead of rejecting
    )
...
class UpdateValidator:
    ...
    def __init__(
        ...
        batch_size: int = 32,
        max_eval_batches: Optional[int] = 8,
    ...
    def _evaluate(
        ...
        val_cache_key = (id(val_data), self.batch_size, self.max_eval_batches)
        if self._val_data_batched is None or self._val_data_cache_key != val_cache_key:

            eval_ds = val_data.batch(self.batch_size)
            if self.max_eval_batches is not None:
                eval_ds = eval_ds.take(int(max(1, self.max_eval_batches)))
            self._val_data_batched = eval_ds.prefetch(1)
            self._val_data_cache_key = val_cache_key
    ...
    def validate_updates(
        ...
        logger.info(
            "Baseline server score (%s): %.4f", self.eval_metric, baseline_score
        )
        ...
        arr_D = _normalise_scalar_to_01(
            _log_scale(np.array(raw_data_vols, dtype=np.float64))
        )
        ...
        raw_scores = (
            w.alpha * arr_G + w.beta * arr_sim + w.gamma * arr_D + w.delta * arr_R
        )
    ...
    def update_reputations(
        ...
        for rec in records:
            ...
            if rec.rejected:
                ...
                logger.debug(
                    "Reputation double-penalty for %s (rejected).", rec.client_id
                )
