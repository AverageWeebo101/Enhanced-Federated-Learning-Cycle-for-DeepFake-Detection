...
class ReputationLedger:
    ...
    def update(self, client_id: str, update_was_beneficial: bool) -> None:
        ...
        logger.debug(
            ...
            client_id,
            old,
            self._scores[client_id],
            update_was_beneficial,
        )
...
class EnhancedClientSelector:
    ...
    def score_clients(self, current_round: int) -> List[Tuple[str, float]]:
        """
        ...
        S = np.array(
            [
                staleness_penalty(
                    self.clients[i].metrics.last_selected_round, current_round
                )
                for i in ids
            ]
        )
    ...
    def select(self, current_round: int) -> List[FederatedClient]:
        """
...
class FederatedRoundRunner:
    ...
    def _fedavg(
        ...
        averaged = [np.zeros(w.shape, dtype=np.float64) for w in global_weights]
    ...
    def _validate_update(
        ...
        results = temp_model.evaluate(
            validation_data.batch(self.local_batch_size),
            verbose=0,
            return_dict=True,
        )
