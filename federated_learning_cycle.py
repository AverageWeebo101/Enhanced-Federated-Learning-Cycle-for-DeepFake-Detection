...
- **Model**: ``efficientnetb4_final.keras``   (EfficientNet‑based binary
...
def partition_data_iid(
    ...
    total = int(tf.data.experimental.cardinality(full_dataset).numpy())
...
class FederatedLearningCycle:
    ...
    def __init__(self, config: Optional[FLCycleConfig] = None) -> None:
        ...
        self._local_train_model: Optional[tf.keras.Model] = None
    ...
    def setup_components(self) -> None:
        ...
        self.validator = UpdateValidator(
            ...
            max_eval_batches=cfg.validator_max_eval_batches,
        )
    ...
    def execute_round(
        ...
        if cfg.enable_distillation and proxy_data is not None:
            ...
            if len(contribution_weights) >= 1:
                ...
                logger.info(
                    "Distillation complete — final loss %.5f",
                    distill_loss,
                )

                # Update validator reference after distillation

                self.validator.global_model.set_weights(self.global_model.get_weights())
    ...
    def run(
        ...
        logger.info(
            "║  Devices: %3d  |  Rounds: %3d  |  Local epochs: %d  ║",
            cfg.num_devices,
            cfg.global_rounds,
            cfg.local_epochs,
        )
        ...
        for t in range(1, cfg.global_rounds + 1):
            ...
            if is_eval_round:
                ...
                is_final_round = t == cfg.global_rounds

                eval_mode = "full" if is_final_round else "lightweight"

                eval_data = test_data if is_final_round else test_data.take(256)

                logger.info("Running %s evaluation (round %d) �", eval_mode, t)

                report = self.evaluator.evaluate(
                    test_data=eval_data,
                    batch_size=cfg.local_batch_size,
                    federated_round=t,
                    latency_max_batches=(None if is_final_round else 4),
                    extra_info={
                    ...
                    max_samples_for_periodic_auc=256,
                )
...
def main() -> None:
    ...
    logger.info(
        "Federated Learning Cycle finished. History keys: %s", list(history.keys())
    )
...
if __name__ == "__main__":
    ...
    # (The real cycle uses efficientnetb4_final.keras via main())
    ...
    print(
        f"Demo model — {demo_model.count_params()} params, "
